...
### 3.10 一键同步上游变更(工作台,2026-09-05 新需求)

上游模型价格经常变动;对**已上线**模型,工作台提供"一键更新模型信息":

- `POST /api/onboarding/sync_from_upstream`:入参 `{model_names?: string[]}`(空=全部已配置模型),复用 3.6 的上游拉取服务取该模型所属渠道的上游 `/api/pricing`
- 与本地现值逐字段 diff:按次价/倍率组/completion/cache/image/audio 倍率、`video_second_price`、`price_tiers`、`billing_mode`+`billing_expr`、`description`(→models 表)
- 校验全过后**直接应用**(写路径=现有 option 编辑通道 + models 表 description 更新);分组独立价与目录手填价格文案不动;catalog 自动文案(3.3)随之自动更新
- 响应返回变更清单 `[{model, field, old, new}]`,前端**突出显示**每处变化并提示"更新成功";confidence 哨兵(37.5/1.0)可疑数据跳过并在结果中单列;billing_expr 应用前过编译冒烟
- 预扣安全:所有写入走既有校验(MaxTierPrice 同级上限、`NormalizePriceTierList`),与手动改价同一路径,无新计费风险

### 3.11 客户端更新通知(2026-09-05 新需求,依赖 3.9 第 2/3 项)

画布客户端要及时感知 kungai 中转站模型信息变化并告知用户——**任何信息变化**(新增、下架、软下线,以及价格/说明/参数表单/schema 等任何字段变更)都触发更新与通知:

- 同步产出**变更报告**:`added[]`/`removed[]`/`soft_disabled[]`(enabled=false 软下线)/`updated[]`(**字段级明细**:模型名 + 变更字段 + 旧值→新值,覆盖价格、说明、limitations、param_schema、schema_override 等);删除对账与同步后刷新是前置(3.9 第 2/3 项)
- 主界面与画布页展示**更新通知**:"本次更新:上新 N 个(名称列表)、下架 M 个、信息变更 K 个",点开看逐字段明细;下次同步前不再重复弹
- 节点选模型下拉**醒目标注两类变化**:新增模型打"新"标,**信息有变更的模型打"更新"标**(角标/高亮,悬停或详情可见变了什么);用户打开过选择框或超过 24h 后清除
- 服务端无需改动:目录全量下发 + `enabled=false` 软下线标记已足够客户端推导全部变更类型(客户端按字段 diff 本地存量即可得 updated 明细)
...
5. **上新工作台**:巡检起草 + 聚合/开闸/忽略接口 + 工作台页面 + 模型管理页"全部起草" + 上游 `/api/pricing` 拉取预填(ratio_sync 服务化复用)+ 一键同步上游变更(3.10,diff+应用+变更高亮)
6. **客户端修复(myhuabua 仓库,独立计划)**:3.9 所列 1–5 项,其中删除对账/同步刷新/commercial 表单为高优;**更新通知与"新"标(3.11)**;与服务端阶段 4 的秒价口径修复配对验证
