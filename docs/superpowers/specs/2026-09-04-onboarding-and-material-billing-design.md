...
画布客户端要及时感知 kungai 中转站模型信息变化并告知用户:

- 同步产出**变更报告**:`added[]`/`removed[]`/`updated[](字段级:价格、说明等)`/`soft_disabled[]`(enabled=false 软下线);删除对账与同步后刷新是前置(3.9 第 2/3 项)
- 主界面与画布页展示**更新通知**:"本次更新:上新 N 个(名称列表)、下架 M 个、信息变更 K 个",点开看明细;下次同步前不再重复弹
- 节点选模型下拉对本次同步新增的模型打"新"标记(用户打开过选择框或超过 24h 后清除)
- 服务端无需改动:目录全量下发 + `enabled=false` 软下线标记已足够客户端推导全部变更类型
